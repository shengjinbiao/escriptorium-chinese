...
- **全局导航「知识库」**：指向 `/gazetteer/` 的地方志知识库页面，主要用于浏览和维护实体、条目及其关系。
- **编辑器顶部「Knowledge」按钮**：在项目或文档视图中打开 `KnowledgeTreeModal`，展示即时生成的知识树、摘要与上下文统计。
- **AI 动作「Generate Knowledge Tree」**：触发后端聚类、实体抽取与摘要流程，为当前项目/文档生成临时知识树数据，可回填到 `KnowledgeTreeModal`，也可以进一步落库到 gazetteer。

三者分工：知识库负责持久化与全局浏览；顶部 Knowledge 提供局部洞察；Generate Knowledge Tree 是生产洞察的管线。后续迭代可以把生成的属性图写入 gazetteer，形成统一的知识服务。

#### 运维备忘：迁移与示例数据

1. **创建或重建 gazetteer 表**
   ```bash
   docker compose exec web python manage.py migrate gazetteer
   ```
   若需完全重置，可先执行 `docker compose exec web python manage.py migrate gazetteer zero` 再重新迁移（注意：这会清空现有知识库数据）。

2. **导入测试数据**
   ```bash
   docker compose exec web python manage.py create_test_data
   ```
   该命令调用 `apps/gazetteer/management/commands/create_test_data.py`，写入示例朝代、方志、实体及其关系，便于在 `/gazetteer/` 页面快速验证。
...
语义检索链路在文档里写得很清楚：先 generate_passages 把行级转录整理成 DocumentPassage，再算向量，最后 index_semantic 写入 ES：
...
"
   ...
   写入ES数据库出错可能的原因是，OpenAI的索引1536维dense_vector，而使用本地的nomic嵌入维度只有768维。所以需要修改变量，并重启容器。进容器验证：docker compose exec web env | grep SEMANTIC 应该能看到两个变量如下：
   ```bash
   $ docker compose exec web env | grep SEMANTIC
   SEMANTIC_EMBEDDING_MODEL=text-embedding-nomic-embed-text-v1.5
   SEMANTIC_EMBEDDING_DIM=768
   ```
   语义检索这套流程实时调用嵌入和生成语义回答，采用本地LM Studio等OpenAI兼容的接口，需要本地服务器常驻，当关闭本地LM Studio的问答和嵌入服务，Django无法生成查询向量或者答案，就会直接抛出500错误。
