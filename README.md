...
3. **使用方式**  
   - 访问 Web 搜索页，输入问题即可看到“AI Answer”与“Semantic Matches”模块；关键词结果仍保留以便对照。  
   - 开发者可直接调用 `POST /api/search/semantic/`，请求体示例：`{"query": "温州有哪些著名古迹？", "limit": 5, "with_answer": true}`。

4. **常见问题**  
   - 若提示缺少依赖，请确认上述 pip 安装步骤已在所有相关容器执行。  
   - 如果返回 503，可检查 Elasticsearch 是否已重建语义索引、OpenAI 凭证是否有效。
